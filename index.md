# 💻 Ján Ročkár
### SAP Business Warehouse & Analytics Cloud

Welcome to my personal technical repository. This space serves as a structured knowledge base where I document issues, edge cases, and optimization strategies across the SAP ecosystem, with a focus on SAP BW, BW/4HANA, ABAP, and SAP Analytics Cloud (SAC).

---

## 📂 Knowledge Base & Case Studies

### 📊 SAP Analytics Cloud & Frontend
* [SAC Excel Add-in – Blank Screen (`office.html`)](cases/SAC/sac-addin.md)
* [Sem len dopíš názov ďalšieho prípadu](cases/SAC/tu-bude-novy-subor.md)

### 📈 SAP BW & BW/4HANA
* [Názov problému s BW](cases/BW/subor-pre-bw.md)

### 💻 ABAP, AMDP & CDS Views
* [Názov optimalizácie SQL query / AMDP](cases/ABAP/subor-pre-abap.md)

---

> 🛠️ *This repository is continuously updated as new edge cases are analyzed and resolved in production environments.*
